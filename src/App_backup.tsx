...
// ─── Dashboard ───────────────────────────────────────────────────────────────
function Dashboard({ products, movements }: { products: Product[]; movements: Movement[] }) {
  ...
  const totalStock = products.filter(p => p.active).reduce((s, p) => s + p.currentStock, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="p-5">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Alertas — Estoque Crítico</p>
          {critical.length === 0 ? (
            <p className="text-sm text-slate-400 py-4 text-center">Nenhum item crítico</p>
          ) : (
            <div className="space-y-2">
              {critical.map(p => {
                const cls = productClasses.find(c => c.id === p.classId);
                const pct = Math.min(100, Math.round((p.currentStock / p.minStock) * 100));
                return (
                  <div key={p.id} className="flex items-center gap-3 py-2 border-b border-slate-50 last:border-0">
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm font-mono font-bold text-red-600">{p.currentStock}</p>
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          {view === 'dashboard' && <Dashboard products={products} movements={movements} />}
